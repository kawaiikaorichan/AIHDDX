...
extern "C"
{
	__declspec(dllexport) __declspec(dllexport) void __cdecl Init(const char* path, const HelperFunctions& helperFunctions)
	{
		...
		DXSS_String = config->getString("Textures", "DXSS", "UpgradedDX");
		std::string KindofTextDC_String = "LikeDX";
		KindofTextDC_String = config->getString("Textures", "DreamcastChars", "LikeDX");
		std::string DCSS_String = "UpgradedLDX";
		DCSS_String = config->getString("Textures", "DreamcastSS", "UpgradedLDX");
		...
		if (LWMural == Preview)
		{
			ReplaceTex("RUIN03", "lost2_wall1", "miscellaneous", "PVMural", 11, 256, 256);
		}

		if (CISpace == Hesse)
		{
			ReplaceTex("ADVSS01", "chaoinspace", "miscellaneous", "ChaoInSpace", 209, 128, 128);
			ReplaceTex("EGM2_TIKEI", "chaoinspace", "miscellaneous", "ChaoInSpace", 213, 128, 128);
		}
